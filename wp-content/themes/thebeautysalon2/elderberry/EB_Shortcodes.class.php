        ...
        if( empty( $content ) ) {
			...
			<div data-slideshow='<?php echo $slideshow ?>' class='tiles' data-col='<?php echo $columns ?>' style="<?php echo $style_string ?>" data-slideshow_speed='<?php echo $slideshow_speed ?>' data-animation_speed='<?php echo $animation_speed ?>' data-animation_order='<?php echo $animation_order ?>'>
				<?php
					...
					$params = str_replace( array( '"', "'" ), '', $tile[1] );
					$params = preg_replace( '/ +/', '&', $params );
					$params = wp_parse_args( $params );


					$tile_background = ( !empty( $params['background'] ) ) ? $this->prepare_color( $params['background'] ) : $background;

				?>
				<div class="tile" style="background-color:<?php echo $tile_background ?>">
					<div class="image">
						<?php
							$image = wp_get_attachment_image_src( $params['image'], 'eb_xlarge_thumb' );
							echo '<img src="' . $image[0] . '">';
						?>
					</div>
					<div class="tile-content">
						<div class='inner'>
							<?php echo do_shortcode($tile[2]) ?>
						</div>
						<?php if( !empty( $params['url'] ) ) : ?>
							<a class="link" href='<?php echo $params['url'] ?>'><span style='background-color: <?php echo $tile_background ?>' class='read-more-arrow'></span></a>
						<?php endif ?>
					</div>
				</div>
				<?php endforeach; ?>
